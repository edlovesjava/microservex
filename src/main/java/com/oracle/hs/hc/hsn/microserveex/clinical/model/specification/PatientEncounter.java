/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package com.oracle.hs.hc.hsn.microserveex.clinical.model.specification;

import com.oracle.hs.hc.hsn.microserveex.common.types.AsRole;
import com.oracle.hs.hc.hsn.microserveex.common.types.TimeInterval;
import com.oracle.hs.hc.hsn.microserveex.enterprise.model.specification.Location;
import java.util.Set;

/**
 *
 * @author edlovesjava
 */
public interface PatientEncounter {
    Patient getPatientEncountered();
    TimeInterval getTimeInterval();
    Set<ClinicalEvent> getEventsOccurred();
    Location getLocationOfEncounter();
    Set<AsRole<ProviderRole, CareProvider>> getParticipatingProviders();
    
}
